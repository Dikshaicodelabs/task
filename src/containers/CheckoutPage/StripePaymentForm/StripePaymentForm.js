...
class StripePaymentForm extends Component {
  constructor(props) {
    ...
    this.state = {
      ...initialState,
      selectedOption: 'token', // Store selected radio button in state
    };
  ...
  handleSubmit(values) {
    ...
    // If card is selected, use default payment handling
    const hasDefaultPaymentMethod = defaultPaymentMethod?.id ?? null;
    const selectedPaymentMethod = getPaymentMethod(
      paymentMethod,
      ensurePaymentMethodCard(defaultPaymentMethod)?.id
    );

    // Ensure the card details are valid before submission
    const { cardValueValid } = this.state;
    const { onetimePaymentNeedsAttention } = checkOnetimePaymentFields(
      cardValueValid,
      selectedPaymentMethod,
      hasDefaultPaymentMethod,
      ...
      return (
        <div>
          <div className={css.radioWrapper}>
            ...
            />
            <label htmlFor="card" className={css.buttonLabel}>
              Card
            </label>
          </div>
          <div className={css.radioWrapper}>
            <input
              ...
              value="token"
              className={css.radioButton}
              checked={this.state.selectedOption === 'token'}
              onChange={this.handleOptionChange}
            />
            <label htmlFor="token" className={css.buttonLabel}>
              Token
            </label>
          </div>
        </div>
                ...
                {userType === 'customer' && this.state.selectedOption === 'token' ? (
                  <>
                    ...
                    <select name="token" id="token">
                      <option value="volvo">{token}</option>
                    </select>
                  </>
                ) : (
                  <OneTimePaymentWithCardElement
